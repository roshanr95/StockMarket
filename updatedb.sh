...
do
  ...
  psql -U karan -d stockmarket -h localhost -c "insert into stock_history values('$company','${array[0]}',${array[1]})"
  psql -U karan -d stockmarket -h localhost -c "update company set curr_stock_price=${array[1]} where ticker_symbol='$company'"
done
